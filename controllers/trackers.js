...
trackersRouter.get("/", async (request, response) => {
  ...
  response.json(
    alerts.map((alert) => ({
      ...
      productPriceDiff:
        alert.product.initial_price - alert.product.current_price,
...
trackersRouter.put("/:id", async (request, response) => {
  const alert = await Alert.findById(request.params.id).populate("user", {
    _id: 1,
  })
  ...
  response.status(200).json({
    ...
    productPriceDiff:
      updatedAlert.product.initial_price - updatedAlert.product.current_price,
...
trackersRouter.delete("/:id", async (request, response) => {
  ...
  const removedAlert = await Alert.findByIdAndRemove(request.params.id)
  return response.status(200).json(removedAlert)
})
